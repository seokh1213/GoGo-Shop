package com.gogo.gogoshop.util;
...
public class AES256Util {
    public static String encrypt(final String key, final String data) {
        try {
            ...
            return new String(Base64.getEncoder().encode(encrypted)).replaceAll("\\s", "+");
        } catch (Exception e) {
    ...
    public static String decrypt(final String key, final String encryptedData) {
        try {
            ...
            byte[] decrypted = Base64.getDecoder().decode(encryptedData.replaceAll("\\s", "+").getBytes(StandardCharsets.UTF_8));
